...
/// View model for the paginated "all user reviews" screen.
///
/// Follows the same cursor-based pagination pattern as `LibraryViewModel`.
@Observable
@MainActor
final class AllUserReviewsViewModel {
    ...
    private func subscribeToPage(userId: String, cursor: String?) {
        repository.subscribeToUserReviewsPaginated(
            userId: userId,
            numItems: 20,
            cursor: cursor
        )?
        ...
        .sink(
            ...
            receiveValue: { [weak self] result in
                ...
                if cursor == nil {
                    self.reviews = result.page
                } else {
                    let existingIds = Set(self.reviews.map(\._id))
                    let newItems = result.page.filter { !existingIds.contains($0._id) }
                    self.reviews.append(contentsOf: newItems)
                }
